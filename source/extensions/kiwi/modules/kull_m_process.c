...
NTSTATUS (WINAPI * NtSuspendProcess)(IN HANDLE ProcessHandle);
NTSTATUS (WINAPI * NtResumeProcess)(IN HANDLE ProcessHandle);
NTSTATUS (WINAPI * NtTerminateProcess)(IN OPTIONAL HANDLE ProcessHandle, IN NTSTATUS ExitStatus);
PPEB (WINAPI * RtlGetCurrentPeb)();
NTSTATUS (WINAPI * RtlAdjustPrivilege)(IN ULONG Privilege, IN BOOL Enable, IN BOOL CurrentThread, OUT PULONG pPreviousState);
NTSTATUS (WINAPI * RtlCreateUserThread)(IN HANDLE Process, IN OPTIONAL PSECURITY_DESCRIPTOR ThreadSecurityDescriptor, IN CHAR Flags, IN OPTIONAL ULONG ZeroBits, IN OPTIONAL SIZE_T MaximumStackSize, IN OPTIONAL SIZE_T CommittedStackSize, IN OPTIONAL PTHREAD_START_ROUTINE StartAddress, IN OPTIONAL PVOID Parameter, OUT OPTIONAL PHANDLE Thread, OUT OPTIONAL PCLIENT_ID ClientId);

VOID kull_m_process_initialise()
{
	HMODULE ntDll = GetModuleHandleA("ntdll");

	NtQuerySystemInformation = (NTSTATUS (WINAPI *)(IN SYSTEM_INFORMATION_CLASS SystemInformationClass, OUT PVOID SystemInformation, IN ULONG SystemInformationLength, OUT OPTIONAL PULONG ReturnLength))GetProcAddress(ntDll, "NtQuerySystemInformation");
	NtQueryInformationProcess = (NTSTATUS (WINAPI *)(IN HANDLE ProcessHandle, IN PROCESSINFOCLASS ProcessInformationClass, OUT PVOID ProcessInformation, OUT ULONG ProcessInformationLength, OUT OPTIONAL PULONG ReturnLength))GetProcAddress(ntDll, "NtQueryInformationProcess");
	NtSuspendProcess = (NTSTATUS (WINAPI *)(IN HANDLE ProcessHandle))GetProcAddress(ntDll, "NtSuspendProcess");
	NtResumeProcess = (NTSTATUS (WINAPI *)(IN HANDLE ProcessHandle))GetProcAddress(ntDll, "NtResumeProcess");
	NtTerminateProcess = (NTSTATUS (WINAPI *)(IN OPTIONAL HANDLE ProcessHandle, IN NTSTATUS ExitStatus))GetProcAddress(ntDll, "NtTerminateProcess");
	RtlGetCurrentPeb = (PPEB (WINAPI *)())GetProcAddress(ntDll, "RtlGetCurrentPeb");
	...
	RtlCreateUserThread = (NTSTATUS (WINAPI *)(IN HANDLE Process, IN OPTIONAL PSECURITY_DESCRIPTOR ThreadSecurityDescriptor, IN CHAR Flags, IN OPTIONAL ULONG ZeroBits, IN OPTIONAL SIZE_T MaximumStackSize, IN OPTIONAL SIZE_T CommittedStackSize, IN OPTIONAL PTHREAD_START_ROUTINE StartAddress, IN OPTIONAL PVOID Parameter, OUT OPTIONAL PHANDLE Thread, OUT OPTIONAL PCLIENT_ID ClientId))GetProcAddress(ntDll, "RtlCreateUserThread");
}
